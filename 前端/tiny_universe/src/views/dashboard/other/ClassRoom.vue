  ...
  <div class="class-room">
    ...
    <el-dialog v-model="isDialogAlive" width="800" center @close="closeDialog">
      ...
      <div v-else-if="isObject === 1">
        ...
        <div v-show="isCommited" style="margin-top: 50px;padding: 50px">
          <el-icon :color="'red'" v-if="option.selected !== problem.t"><CloseBold /></el-icon>
          <el-icon :color="'yellow'" v-else><Select /></el-icon>
          <p>答案解析：{{problem.p}}</p>
        </div>
      </div>
      <div v-else>
...
const closeProblem = () => {
  Object.assign(problem,{content:'',A:'',B:'',C:'',D:'',P:''})
  option.selected = 'A';
...
const toProblem = (type) => {
  ...
  isObject.value = 1;
  doGet('/gpt/produceProblem',{type:type}).then((resp) => {
